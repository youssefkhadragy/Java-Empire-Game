		...
		for (City c : controlledCities) {
			if (c.getName().equals(cityName)) {
				for (MilitaryBuilding b : c.getMilitaryBuildings()) {
					if(b.isOverDrive()) {
						int currMaxRec = b.getMaxRecruit();
						b.setMaxRecruit(2 * currMaxRec);
						if ((type.toLowerCase().equals("archer") && b instanceof ArcheryRange)|| (type.toLowerCase().equals("cavalry") && b instanceof Stable)|| (type.toLowerCase().equals("infantry") && b instanceof Barracks)) {
							Unit u = b.recruit();
							Unit u2 = b.recruit();
							if (treasury < 4*b.getRecruitmentCost()) {
								throw new NotEnoughGoldException("Not enough gold");
							}
							treasury -= 4*b.getRecruitmentCost();
							u.setParentArmy(c.getDefendingArmy());
							u2.setParentArmy(c.getDefendingArmy());
							c.getDefendingArmy().getUnits().add(u);
							c.getDefendingArmy().getUnits().add(u2);
							b.setMaxRecruit(currMaxRec);
						}
					}
					else if(!b.isOverDrive()) {
					if ((type.toLowerCase().equals("archer") && b instanceof ArcheryRange)|| (type.toLowerCase().equals("cavalry") && b instanceof Stable)|| (type.toLowerCase().equals("infantry") && b instanceof Barracks)) {
						Unit u = b.recruit();
